...
router.get('/check-warranty/:machineId/:partId', async (req, res) => {
  try {
    ...
    const machineUnderWarranty = machine.warrantyEndDate && currentDate <= new Date(machine.warrantyEndDate);
    const partUnderWarranty = part.warrantyEndDate && currentDate <= new Date(part.warrantyEndDate);
...
router.put('/update-maintenance-status/:machineId/:partId', async (req, res) => {
  try {
    const { machineId, partId } = req.params;
